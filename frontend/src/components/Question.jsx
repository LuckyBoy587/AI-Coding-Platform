...
const Question = ({onRequestSent}) => {
  const {code, userInput, setUserInput} = useCode();
  ...
  return (
    <div
      ...
      ></textarea>
      <div
        className={"flex bg-[var(--color-bg-primary)] text-[var(--color-text-secondary)] rounded-lg border border-[var(--color-text-secondary)] transition-colors duration-200 min-w-0"}
        onKeyDown={(e) => {
          if (e.key === "Enter") {
            e.preventDefault();
            handleAskGemini();
